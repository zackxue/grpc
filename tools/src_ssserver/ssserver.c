...
grpcMethod_t ssserver_methodList[] = {
		{"account_get_users"             , USERDEF_RESP_ssserver_account_get_users         , SERVER_ssserver_account_get_users               , GRPC_USER_LEVEL_Administrator   },
		{"account_add_user"              , USERDEF_RESP_ssserver_account_add_user          , SERVER_ssserver_account_add_user                , GRPC_USER_LEVEL_Administrator   },
		{"account_del_user"              , USERDEF_RESP_ssserver_account_del_user          , SERVER_ssserver_account_del_user                , GRPC_USER_LEVEL_Administrator   },
		{"account_modify_user"           , USERDEF_RESP_ssserver_account_modify_user       , SERVER_ssserver_account_modify_user             , GRPC_USER_LEVEL_Administrator   },
		{"channel_get"                   , USERDEF_RESP_ssserver_channel_get               , SERVER_ssserver_channel_get                     , GRPC_USER_LEVEL_Administrator   },
		{"channel_add"                   , USERDEF_RESP_ssserver_channel_add               , SERVER_ssserver_channel_add                     , GRPC_USER_LEVEL_Administrator   },
		{"channel_remove"                , USERDEF_RESP_ssserver_channel_remove            , SERVER_ssserver_channel_remove                  , GRPC_USER_LEVEL_Administrator   },
		{"channel_modify"                , USERDEF_RESP_ssserver_channel_modify            , SERVER_ssserver_channel_modify                  , GRPC_USER_LEVEL_Administrator   },
		{"broadcast_discovery"           , USERDEF_RESP_ssserver_broadcast_discovery       , SERVER_ssserver_broadcast_discovery             , GRPC_USER_LEVEL_Administrator   },
		{"broadcast_get_inet"            , USERDEF_RESP_ssserver_broadcast_get_inet        , SERVER_ssserver_broadcast_get_inet              , GRPC_USER_LEVEL_Administrator   },
		{"broadcast_eth_set"             , USERDEF_RESP_ssserver_broadcast_eth_set         , SERVER_ssserver_broadcast_eth_set               , GRPC_USER_LEVEL_Administrator   },
		{"broadcast_ppp_set"             , USERDEF_RESP_ssserver_broadcast_ppp_set         , SERVER_ssserver_broadcast_ppp_set               , GRPC_USER_LEVEL_Administrator   },
		{"broadcast_wifi_connect"        , USERDEF_RESP_ssserver_broadcast_wifi_connect    , SERVER_ssserver_broadcast_wifi_connect          , GRPC_USER_LEVEL_Administrator   },
		{"broadcast_wifi_list_ap"        , USERDEF_RESP_ssserver_broadcast_wifi_list_ap    , SERVER_ssserver_broadcast_wifi_list_ap          , GRPC_USER_LEVEL_Administrator   },
		{"storage_get_info"              , USERDEF_RESP_ssserver_storage_get_info          , SERVER_ssserver_storage_get_info                , GRPC_USER_LEVEL_Administrator   },
		{"storage_start_intial_disk"     , USERDEF_RESP_ssserver_storage_start_intial_disk , SERVER_ssserver_storage_start_intial_disk       , GRPC_USER_LEVEL_Administrator   },
		{"storage_get_disk_initial_rate" , USERDEF_RESP_ssserver_storage_get_disk_initial_rate, SERVER_ssserver_storage_get_disk_initial_rate   , GRPC_USER_LEVEL_Administrator   },
		{"storage_get_record_fragments"  , USERDEF_RESP_ssserver_storage_get_record_fragments, SERVER_ssserver_storage_get_record_fragments    , GRPC_USER_LEVEL_Administrator   },
		{"stream_get_service"            , USERDEF_RESP_ssserver_stream_get_service        , SERVER_ssserver_stream_get_service              , GRPC_USER_LEVEL_Administrator   },
		{NULL, NULL, NULL}
};
...
int CLIENT_ssserver_account_get_users(grpc_t *grpc, PARAM_REQ_ssserver_account_get_users *req, PARAM_RESP_ssserver_account_get_users *resp)
{
	...
	CLIENT_REQ_ssserver_account_get_users(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_account_get_users(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_account_get_users req;
	PARAM_RESP_ssserver_account_get_users resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));

	ret = USERDEF_ssserver_account_get_users(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_account_add_user(grpc_t *grpc, PARAM_REQ_ssserver_account_add_user *req, PARAM_RESP_ssserver_account_add_user *resp)
{
	...
	CLIENT_REQ_ssserver_account_add_user(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_account_add_user(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_account_add_user req;
	PARAM_RESP_ssserver_account_add_user resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_account_add_user(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_account_del_user(grpc_t *grpc, PARAM_REQ_ssserver_account_del_user *req, PARAM_RESP_ssserver_account_del_user *resp)
{
	...
	CLIENT_REQ_ssserver_account_del_user(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_account_del_user(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_account_del_user req;
	PARAM_RESP_ssserver_account_del_user resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_account_del_user(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_account_modify_user(grpc_t *grpc, PARAM_REQ_ssserver_account_modify_user *req, PARAM_RESP_ssserver_account_modify_user *resp)
{
	...
	CLIENT_REQ_ssserver_account_modify_user(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_account_modify_user(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_account_modify_user req;
	PARAM_RESP_ssserver_account_modify_user resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_account_modify_user(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_channel_get(grpc_t *grpc, PARAM_REQ_ssserver_channel_get *req, PARAM_RESP_ssserver_channel_get *resp)
{
	...
	CLIENT_REQ_ssserver_channel_get(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_channel_get(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_channel_get req;
	PARAM_RESP_ssserver_channel_get resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));

	ret = USERDEF_ssserver_channel_get(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_channel_add(grpc_t *grpc, PARAM_REQ_ssserver_channel_add *req, PARAM_RESP_ssserver_channel_add *resp)
{
	...
	CLIENT_REQ_ssserver_channel_add(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_channel_add(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_channel_add req;
	PARAM_RESP_ssserver_channel_add resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_channel_add(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_channel_remove(grpc_t *grpc, PARAM_REQ_ssserver_channel_remove *req, PARAM_RESP_ssserver_channel_remove *resp)
{
	...
	CLIENT_REQ_ssserver_channel_remove(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_channel_remove(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_channel_remove req;
	PARAM_RESP_ssserver_channel_remove resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_channel_remove(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_channel_modify(grpc_t *grpc, PARAM_REQ_ssserver_channel_modify *req, PARAM_RESP_ssserver_channel_modify *resp)
{
	...
	CLIENT_REQ_ssserver_channel_modify(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_channel_modify(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_channel_modify req;
	PARAM_RESP_ssserver_channel_modify resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_channel_modify(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_broadcast_discovery(grpc_t *grpc, PARAM_REQ_ssserver_broadcast_discovery *req, PARAM_RESP_ssserver_broadcast_discovery *resp)
{
	...
	CLIENT_REQ_ssserver_broadcast_discovery(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_broadcast_discovery(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_broadcast_discovery req;
	PARAM_RESP_ssserver_broadcast_discovery resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));

	ret = USERDEF_ssserver_broadcast_discovery(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_broadcast_get_inet(grpc_t *grpc, PARAM_REQ_ssserver_broadcast_get_inet *req, PARAM_RESP_ssserver_broadcast_get_inet *resp)
{
	...
	CLIENT_REQ_ssserver_broadcast_get_inet(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_broadcast_get_inet(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_broadcast_get_inet req;
	PARAM_RESP_ssserver_broadcast_get_inet resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));

	ret = USERDEF_ssserver_broadcast_get_inet(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_broadcast_eth_set(grpc_t *grpc, PARAM_REQ_ssserver_broadcast_eth_set *req, PARAM_RESP_ssserver_broadcast_eth_set *resp)
{
	...
	CLIENT_REQ_ssserver_broadcast_eth_set(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_broadcast_eth_set(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_broadcast_eth_set req;
	PARAM_RESP_ssserver_broadcast_eth_set resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_broadcast_eth_set(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_broadcast_ppp_set(grpc_t *grpc, PARAM_REQ_ssserver_broadcast_ppp_set *req, PARAM_RESP_ssserver_broadcast_ppp_set *resp)
{
	...
	CLIENT_REQ_ssserver_broadcast_ppp_set(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_broadcast_ppp_set(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_broadcast_ppp_set req;
	PARAM_RESP_ssserver_broadcast_ppp_set resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_broadcast_ppp_set(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_broadcast_wifi_connect(grpc_t *grpc, PARAM_REQ_ssserver_broadcast_wifi_connect *req, PARAM_RESP_ssserver_broadcast_wifi_connect *resp)
{
	...
	CLIENT_REQ_ssserver_broadcast_wifi_connect(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_broadcast_wifi_connect(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_broadcast_wifi_connect req;
	PARAM_RESP_ssserver_broadcast_wifi_connect resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_broadcast_wifi_connect(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_broadcast_wifi_list_ap(grpc_t *grpc, PARAM_REQ_ssserver_broadcast_wifi_list_ap *req, PARAM_RESP_ssserver_broadcast_wifi_list_ap *resp)
{
	...
	CLIENT_REQ_ssserver_broadcast_wifi_list_ap(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_broadcast_wifi_list_ap(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_broadcast_wifi_list_ap req;
	PARAM_RESP_ssserver_broadcast_wifi_list_ap resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_broadcast_wifi_list_ap(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_storage_get_info(grpc_t *grpc, PARAM_REQ_ssserver_storage_get_info *req, PARAM_RESP_ssserver_storage_get_info *resp)
{
	...
	CLIENT_REQ_ssserver_storage_get_info(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_storage_get_info(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_storage_get_info req;
	PARAM_RESP_ssserver_storage_get_info resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));

	ret = USERDEF_ssserver_storage_get_info(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_storage_start_intial_disk(grpc_t *grpc, PARAM_REQ_ssserver_storage_start_intial_disk *req, PARAM_RESP_ssserver_storage_start_intial_disk *resp)
{
	...
	CLIENT_REQ_ssserver_storage_start_intial_disk(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_storage_start_intial_disk(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_storage_start_intial_disk req;
	PARAM_RESP_ssserver_storage_start_intial_disk resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_storage_start_intial_disk(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_storage_get_disk_initial_rate(grpc_t *grpc, PARAM_REQ_ssserver_storage_get_disk_initial_rate *req, PARAM_RESP_ssserver_storage_get_disk_initial_rate *resp)
{
	...
	CLIENT_REQ_ssserver_storage_get_disk_initial_rate(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_storage_get_disk_initial_rate(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_storage_get_disk_initial_rate req;
	PARAM_RESP_ssserver_storage_get_disk_initial_rate resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_storage_get_disk_initial_rate(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_storage_get_record_fragments(grpc_t *grpc, PARAM_REQ_ssserver_storage_get_record_fragments *req, PARAM_RESP_ssserver_storage_get_record_fragments *resp)
{
	...
	CLIENT_REQ_ssserver_storage_get_record_fragments(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_storage_get_record_fragments(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_storage_get_record_fragments req;
	PARAM_RESP_ssserver_storage_get_record_fragments resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_storage_get_record_fragments(grpc, &req, &resp);
	if (ret != 0)
		return ret;
...
int CLIENT_ssserver_stream_get_service(grpc_t *grpc, PARAM_REQ_ssserver_stream_get_service *req, PARAM_RESP_ssserver_stream_get_service *resp)
{
	...
	CLIENT_REQ_ssserver_stream_get_service(grpc, req);
	ret = grpc_c_recv(grpc);
	if (0 == ret)
	{
		ret = CLIENT_RESP_ssserver_stream_get_service(grpc, resp);
	}
...
{
	int ret;
	PARAM_REQ_ssserver_stream_get_service req;
	PARAM_RESP_ssserver_stream_get_service resp;
	__GENERATE_FUNC_DEBUG__();
	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));
	...
	ret = USERDEF_ssserver_stream_get_service(grpc, &req, &resp);
	if (ret != 0)
		return ret;
